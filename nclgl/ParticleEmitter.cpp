...
ParticleEmitter::ParticleEmitter(void) {
	...
	*/
	...
	texture = SOIL_load_OGL_texture(TEXTUREDIR"blackSmoke00.PNG",
		SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_COMPRESS_TO_DXT);
}
...
Particle* ParticleEmitter::GetFreeParticle() {
	...
	p->colour = Vector4(0.8f, 0.8f, 0.8f, 1.0);
	...
	p->direction.x += ((RAND() - RAND()) * particleVariance);
	p->direction.y += (2.0f * particleVariance);
	p->direction.z += ((RAND() - RAND()) * particleVariance);

	p->direction.Normalise();	//Keep its direction normalised!


	// MAke position = vec3. origin  MAKE = NEW VARIABLE
	//p->position.ToZero();
	//p->position = Position;
	p->position = Vector3(RAND() * Position.x, Position.y, RAND()*Position.z);
